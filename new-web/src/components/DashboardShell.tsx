...
/** Case detail uses the landing-style full-width layout (no sidebar). */
function isCaseDetailPath(pathname: string) {
  return /^\/dashboard\/case\//.test(pathname);
}

export function DashboardShell({ children }: { children: ReactNode }) {
  const pathname = usePathname() ?? "";
  const caseDetail = isCaseDetailPath(pathname);

  if (caseDetail) {
    return (
      <div className="landing-page min-h-dvh overflow-y-auto bg-black">
        {children}
      </div>
    );
  }
