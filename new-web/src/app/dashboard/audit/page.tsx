...
export default function AuditPage() {
  ...
  return (
    <div className="p-6 space-y-5" style={{ background: "var(--color-bg)", minHeight: "100%" }}>
      <div>
        <h1 className="text-xl font-bold" style={{ color: "var(--color-text)" }}>Call Log</h1>
        <p className="text-sm mt-0.5" style={{ color: "var(--color-muted)" }}>
          {MOCK_AUDIT_CALLS.length} calls placed
        </p>
      </div>
      ...
      >
        <table className="w-full border-collapse text-sm" aria-label="Call audit log">
          <thead>
            <tr style={{ background: "var(--color-surface-2)", borderBottom: "1px solid var(--color-border)" }}>
              {["Time", "Patient", "Language", "Duration", "Outcome", ""].map((h) => (
                <th
                  key={h}
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <td className="py-3 px-4 font-mono text-sm" style={{ color: "var(--color-text)" }}>
                    ...
                    <td className="py-3 px-4">
                      <button
                        ...
                        aria-label={isOpen ? "Close transcript" : "View transcript"}
                      >
                        {isOpen ? "Close" : "View →"}
                      </button>
                    </td>
                  ...
                  {isOpen && (
                    <tr key={`${call.id}-detail`} style={{ background: "var(--color-surface-2)" }}>
                      <td colSpan={6} className="px-4 pb-4 pt-2">
                        <div className="space-y-3">
                          ...
                          <div
                            ...
                            style={{ border: "1px solid var(--color-border)", background: "var(--color-surface)" }}
                          >
                            <pre className="font-mono text-xs leading-relaxed whitespace-pre-wrap" style={{ color: "var(--color-text)" }}>
                          ...
                          {call.audioUrl && (
                            <div>
                              ...
                              <audio controls src={call.audioUrl} className="w-full h-8" />
                            </div>
                          )}
                        </div>
                      </td>
